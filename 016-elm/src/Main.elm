...
import String exposing (length, left, dropLeft)

import BigInt exposing (BigInt, toString, fromInt, fromIntString, add, mul, sub, compare)
...
nthPowerHelper acc n m =
  if m == fromInt 1 then acc else nthPowerHelper (mul acc n) n (sub m (fromInt 1))
...
digitSumHelper acc nStr =
  if length nStr == 0 then acc else
